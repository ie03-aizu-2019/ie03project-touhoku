	...
	public void Sort() {
		for (int i = 0; i < cp.size() - 1; i++) {
			for (int j = cp.size() - 1; j > i; j--) {
				if (cp.get(i).pointInfo.type == e_PointType.CrossPoint
						&& cp.get(j).pointInfo.type == e_PointType.CrossPoint) {
	...
	public void AddMovableCrossPoint() {
		for (int i = 0; i < cp.size(); i++) {
			for (int j = 0; j < cp.size(); j++) {
				if (i != j) {
					if (cp.get(i).line1 == cp.get(j).line1 || cp.get(i).line1 == cp.get(j).line2
							|| cp.get(i).line2 == cp.get(j).line2) {
